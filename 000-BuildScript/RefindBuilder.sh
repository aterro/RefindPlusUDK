...
XCODE_DIR_NPT="${EDK2_DIR}/Build/Refind/NOOPT_XCODE5"
...
if [ "${RUN_DBG}" == 'True' ] ; then
    ...
    pushd "${EDK2_DIR}" > /dev/null || runErr "${ErrMsg}"
    source edksetup.sh BaseTools
    build -a X64 -b DEBUG -t XCODE5 -p RefindPkg/RefindPkg.dsc
    if [ -d "${EDK2_DIR}/Build" ] ; then
...
if [ "${RUN_NPT}" == 'True' ] ; then
    ...
    pushd "${EDK2_DIR}" > /dev/null || runErr "${ErrMsg}"
    source edksetup.sh BaseTools
    build -a X64 -b NOOPT -t XCODE5 -p RefindPkg/RefindPkg.dsc
    if [ -d "${EDK2_DIR}/Build" ] ; then
